...
double Vector2::Norm(double eps) const {
    ...
    if (tmp1 < eps && tmp2 < eps)
        return 0;
    return tmp1 > tmp2 ? tmp1 * sqrt(1 + sqr(data[1] / data[0])) : tmp2 * sqrt(1 + sqr(data[0] / data[1]));
}
...
Rotation Rotation::RPY(double roll, double pitch, double yaw) {
    double ca1 = cos(yaw);
    double sa1 = sin(yaw);
    double cb1 = cos(pitch);
    double sb1 = sin(pitch);
    double cc1 = cos(roll);
    double sc1 = sin(roll);
...
{
    Vector axis{};
    double angle = Rotation::GetRotAngle(axis, epsilon);
    return axis * angle;
}
...
double Rotation::GetRotAngle(Vector &axis, double eps) const {
    ...
    if ((std::abs(data[1] - data[3]) < epsilon) && (std::abs(data[2] - data[6]) < epsilon) && (std::abs(data[5] - data[7]) < epsilon)) {
        // singularity found
        // first check for identity matrix which must have +1 for all terms
        //  in leading diagonal and zero in other terms
        if ((std::abs(data[1] + data[3]) < epsilon2) && (std::abs(data[2] + data[6]) < epsilon2) && (std::abs(data[5] + data[7]) < epsilon2) && (std::abs(data[0] + data[4] + data[8] - 3) < epsilon2)) {

...
static inline void Twist_to_Eigen(const KDL::Twist &t, Eigen::MatrixBase<Derived> &e) {
    e(0) = t.vel.data[0];
    e(1) = t.vel.data[1];
    e(2) = t.vel.data[2];
    e(3) = t.rot.data[0];
...
ChainIkSolverPos_LMA::ChainIkSolverPos_LMA(
    ...
    double _eps,
    int _maxiter,
    double _eps_joints) : chain(_chain),
...
const char *ChainIkSolverPos_LMA::strError(const int error) const {
    if (E_GRADIENT_JOINTS_TOO_SMALL == error)
        return "The gradient of E towards the joints is to small";
    else if (E_INCREMENT_JOINTS_TOO_SMALL == error)
        return "The joint position increments are to small";
    else

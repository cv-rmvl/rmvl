...
public:
    ...
    VectorAcc(const VectorAcc &arg) : p(arg.p), v(arg.v), dv(arg.dv) {}
...
public:
    RotationAcc() : R(), w(), dw() {}
    explicit RotationAcc(const Rotation &_R) : R(_R), w(Vector::Zero()), dw(Vector::Zero()) {}
    RotationAcc(const Rotation &_R, const Vector &_w, const Vector &_dw) : R(_R), w(_w), dw(_dw) {}
    RotationAcc(const RotationAcc &arg) : R(arg.R), w(arg.w), dw(arg.dw) {}
...
public:
    ...
    FrameAcc(const Frame &_T, const Twist &_t, const Twist &_dt) : M(_T.M, _t.rot, _dt.rot), p(_T.p, _t.vel, _dt.vel) {}
...
template <>
struct std::hash<KDL::RotationAcc> {

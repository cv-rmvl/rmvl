...
TEST(OPC_UA_ClientTest, connect) {
    UserConfig config;
    Server srv(4999, "Test Server", {{"admin", "admin"}});
    std::thread t(&Server::spin, &srv);
    std::this_thread::sleep_for(50ms);

    Client cli1("opc.tcp://127.0.0.1:4999", {"admin", "admin"});
    EXPECT_TRUE(cli1.ok());
    Client cli2("opc.tcp://127.0.0.1:4999", {"admin", "123456"});
...
static void configServer(Server &srv) {
    // 添加单变量节点
    Variable single_value = 42;
    ...
    // 添加数组变量节点
    Variable variable = std::vector{1, 2, 3, 4, 5};
    ...
    // 添加数据源变量节点
    static int dv_src{};
    DataSourceVariable dv;
    dv.browse_name = "data_source";
    dv.display_name = "this is data source";
    dv.description = "单值（数据源）";
    dv.access_level = VARIABLE_READ | VARIABLE_WRITE;
    dv.on_read = [](const NodeId &) -> Variable { return dv_src; };
    dv.on_write = [](const NodeId &, const Variable &val) { dv_src = val; };
    srv.addDataSourceVariableNode(dv);
    // 添加加法方法节点
    Method method = [](const NodeId &, const Variables &input) {
        int a = input[0], b = input[1];
        Variables output = {a + b};
        return std::make_pair(true, output);
    };
    method.browse_name = "add";
    method.description = "this is add method";
    method.display_name = "加法";
    method.iargs = {{"a", tpInt32}, {"b", tpInt32}};
    method.oargs = {{"c", tpInt32}};
    ...
    name.description = "名字";
    Method get = [](const NodeId &, const Variables &) {
        Variables output = {"Alice"};
        return std::make_pair(true, output);
    };
    get.browse_name = "get";
    get.display_name = "Get";
    get.description = "获取名字";
    get.oargs = {{"name", tpString, 1, "名字"}};
    name.add(get);
...
TEST(OPC_UA_ClientTest, path_search) {
    ...
    Client cli("opc.tcp://127.0.0.1:5000");
    // 读取测试服务器上的变量值
    auto id = cli.find("array");
    Variable variable = cli.read(id);
...
TEST(OPC_UA_ClientTest, variable_io) {
    ...
    std::thread t(&Server::spin, &srv);
    std::this_thread::sleep_for(10ms);
    Client cli("opc.tcp://127.0.0.1:5001");
    // 读取测试服务器上的变量值
    auto id = cli.find("single");
    EXPECT_TRUE(cli.write(id, 99));
    Variable variable = cli.read(id);
    EXPECT_FALSE(variable.empty());
    int single_value = Variable::cast<int>(variable);
...
TEST(OPC_UA_ClientTest, call) {
    ...
    std::thread t(&Server::spin, &srv);
    std::this_thread::sleep_for(10ms);
    Client cli("opc.tcp://127.0.0.1:5002");
    // 调用测试服务器上的方法
    Variables input = {1, 2};
    auto [res, output] = cli.call("add", input);
    EXPECT_TRUE(res);
    EXPECT_EQ(Variable::cast<int>(output[0]), 3);
...
TEST(OPC_UA_ClientTest, callx) {
    ...
    std::thread t(&Server::spin, &srv);
    std::this_thread::sleep_for(10ms);
    Client cli("opc.tcp://127.0.0.1:5003");
    // 以 callx 调用测试服务器上的方法
...
TEST(OPC_UA_ClientTest, findcall) {
    ...
    std::thread t(&Server::spin, &srv);
    std::this_thread::sleep_for(10ms);
    Client cli("opc.tcp://127.0.0.1:5004");
...
TEST(OPC_UA_ClientTest, variable_monitor) {
    ...
    std::thread t(&Server::spin, &srv);
    std::this_thread::sleep_for(10ms);
    Client cli("opc.tcp://127.0.0.1:5005");
    // 订阅测试服务器上的变量
    int receive_data{};
    auto node_id = cli.find("single");
    auto on_change = [&](ClientView, const Variable &value) {
        receive_data = value;
    };
...
TEST(OPC_UA_ClientTest, event_monitor) {
    ...
    Client cli("opc.tcp://127.0.0.1:5010");

    std::string source_name{};
    std::string messgae{};
    int aaa{};
    cli.monitor({"Message", "SourceName", "aaa"}, [&](ClientView, const Variables &fields) {
        messgae = fields[0].cast<std::string>();
        source_name = fields[1].cast<std::string>();
        aaa = fields[2];
    });
    // 触发事件
    auto event = Event::makeFrom(etype);
...
TEST(OPC_UA_ClientTest, timer_test) {
    ...
    std::thread t(&Server::spin, &srv);
    std::this_thread::sleep_for(10ms);
    Client cli("opc.tcp://127.0.0.1:5015");
    int times{};
    auto timer = ClientTimer(cli, 10, [&]() { times++; });
...
TEST(OPC_UA_ClientTest, data_source_variable_IO) {
    ...
    std::thread t(&Server::spin, &srv);
    std::this_thread::sleep_for(10ms);
    Client cli("opc.tcp://127.0.0.1:5020");
    // 读取测试服务器上的变量值

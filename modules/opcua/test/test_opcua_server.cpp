...
TEST(OPC_UA_Server, add_variable_node) {
    Server srv(4810, "TestServer");
    Variable variable = 3.1415;
...
TEST(OPC_UA_Server, add_data_source_variable_node) {
    int data_source{};
    Server srv(4825, "TestServer");
    DataSourceVariable v;
    v.browse_name = v.display_name = "test_int";
    v.description = "this is test int";
    v.on_read = [&](const NodeId &) -> Variable { return data_source; };
    v.on_write = [&](const NodeId &, const Variable &val) { data_source = val.cast<int>(); };
...
TEST(OPC_UA_Server, add_variable_type_node) {
    Server srv(4830);
    VariableType variable_type = "string_test";
...
TEST(OPC_UA_Server, add_method_node) {
    Server srv(4832);
    Method method = [](const NodeId &, const Variables &) -> std::pair<bool, Variables> { return {true, {}}; };
...
TEST(OPC_UA_Server, add_object_node_with_dsv) {
    ...
    object.description = "this is test object";
    object.display_name = "测试对象";
    DataSourceVariable dsv1;
    dsv1.browse_name = "test_dsv1";
    dsv1.description = "this is test data source variable";
    dsv1.display_name = "测试数据源变量 1";
    dsv1.access_level = VARIABLE_READ | VARIABLE_WRITE;
    int src_val{};
    dsv1.on_read = [&](const NodeId &) -> Variable { return src_val; };
    dsv1.on_write = [&](const NodeId &, const Variable &val) { src_val = val; };
    object.add(dsv1);
    auto id = srv.addObjectNode(object);
    EXPECT_FALSE(id.empty());
    srv.spinOnce();
...
TEST(OPC_UA_Server, add_object_node_with_method) {
    ...
    method.description = "this is test method";
    method.display_name = "测试方法";
    method.func = [](const NodeId &, const Variables &) -> std::pair<bool, Variables> { return {true, {}}; };
    object.add(method);
    auto id = srv.addObjectNode(object);
    EXPECT_FALSE(id.empty());
    srv.spinOnce();
...
TEST(OPC_UA_Server, create_object_by_object_type) {
    ...
    auto object = Object::makeFrom(object_type);
    object.browse_name = "test_object";
    object.description = "this is test object";
    object.display_name = "测试对象";
    auto id = srv.addObjectNode(object);
...
TEST(OPC_UA_Server, add_event_type_node) {
    ...
    auto target = srv.find("test_event_type", nodeBaseEventType);
    EXPECT_EQ(id, target);
    srv.spinOnce();
}
...
TEST(OPC_UA_Server, trigger_event) {
    ...
    // 创建事件
    auto event = Event::makeFrom(event_type);
...
TEST(OPC_UA_Server, view_node) {
    ...
    auto target_view_id = nodeViewsFolder | srv.node("test_view");
...
TEST(OPC_UA_Server, timer_test) {
    ...
    auto timer = ServerTimer(srv, 10, [&](ServerView sv) {

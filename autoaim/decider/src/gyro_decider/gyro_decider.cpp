...
{
    string type_priority = gyro_decider_param.TYPE_PRIORITY;
 ...
 */
static void calcPrediction(const group_ptr &p_group, const tracker_ptr &p_tracker,
                           Vec3f translation_dp, float rotangle, Point2f &point_p2d, Point3f &point_p3d, Point2f &point_dp)
{
    gyro_group_ptr p_gyro_group = GyroGroup::cast(p_group);
    ...
    Matx33f rot = {c, 0, s,
                   0, 1, 0,
                   -s, 0, c};
    Vec3f rotation_dp = rot * center2combo_pose - center2combo_pose;
    // 运动合成，计算 3D 预测增量
    Vec3f motion_dp = translation_dp + rotation_dp;
    Vec3f motion_p = tvec + motion_dp;
    // 陀螺仪坐标系转相机坐标系
 ...
 * @return 高速状态下的基础响应
 */
static Point2f calculateHighSpeedBasicResponse(const group_ptr &target_group, const tracker_ptr &target_tracker,
                                               const Point3f &predict_center3d)
{
    // pitch 最大振幅角度
    ...
    // pitch 目标角度
    return {angle_tmp.x, gyro_decider_param.PITCH_RESPONSE_DELAY * (pitch_max * gyro_decider_param.PITCH_RESPONSE_AMP +
                                                                    pitch_min * (1 - gyro_decider_param.PITCH_RESPONSE_AMP))};
}

DecideInfo GyroDecider::decide(const std::vector<group_ptr> &groups, RMStatus,
                               ...
                               const CompensateInfo &compensate_info, const PredictInfo &predict_info)
{
    ...
    // -------------------------【选择目标序列组】-------------------------
    // 初始化目标序列组
    group_ptr target_group = nullptr;
    ...
    if (info.target)
    {
        ...
        gyro_tracker_ptr p_gyro_tracker = GyroTracker::cast(info.target);
        ...
        auto tdKt = Vec3f(pKt(POS_X), pKt(POS_Y), pKt(POS_Z));
        auto tdB = Vec3f(pB(POS_X), pB(POS_Y), pB(POS_Z));
        ...
        {
            // --------------------【提取用于响应的预测量】--------------------
            Point2f resp_p2d; // 预测值 (2D)
            Point3f resp_p3d; // 预测值 (3D)
            Point2f resp_dp;  // 预测增量

            calcPrediction(target_group, info.target, tdKt + tdB, pKt(ANG_Y) + pB(ANG_Y),
                           resp_p2d, resp_p3d, resp_dp);
            // --------------------【提取用于发弹的预测量】--------------------
            Point2f shoot_p2d; // 预测值 (2D)
            Point3f shoot_p3d; // 预测值 (3D)
            Point2f shoot_dp;  // 预测增量
        ...
        {
            // --------------------【提取用于响应的预测量】--------------------
            Point2f resp_x_p2d; // yaw 预测值 (2D)
            Point3f resp_x_p3d; // yaw 预测值 (3D)
            Point2f resp_x_dp;  // yaw 预测增量
            calcPrediction(target_group, info.target, tdKt + tdB, 0, resp_x_p2d, resp_x_p3d, resp_x_dp);

            Point2f resp_y_p2d; // pitch 预测值 (2D)
            Point3f resp_y_p3d; // pitch 预测值 (3D)
            Point2f resp_y_dp;  // pitch 预测增量
            calcPrediction(target_group, info.target, tdKt + tdB, pKt(ANG_Y) + pB(ANG_Y) + pBs(ANG_Y),
                           resp_y_p2d, resp_y_p3d, resp_y_dp);

            // --------------------【提取用于发弹的预测量】--------------------
            Point2f shoot_p2d; // 动态预测值 (2D)
            Point3f shoot_p3d; // 动态预测值 (3D)
            Point2f shoot_dp;  // 动态预测增量
            ...
            info.exp_angle += Point2f(resp_x_dp.x, resp_y_dp.y); // 考虑预测
            info.exp_angle += comp;                              // 考虑补偿
            info.exp_center2d = shoot_p2d;
            info.exp_center3d = shoot_p3d;
        }
        ...
        if (info.target != nullptr)
        {
            ...
            if (getDistance(info.exp_center2d, info.shoot_center) <=
                gyro_decider_param.NORMAL_RADIUS_RATIO * info.target->front()->getHeight())
                info.can_shoot = true;
        }
    }
    return info;
...
tracker_ptr GyroDecider::getClosestTracker(const group_ptr &p_group, const PredictInfo &info)
{
    if (p_group == nullptr)
        return nullptr;
    if (p_group->empty())
...
group_ptr GyroDecider::getHighestPriorityGroup(const std::vector<group_ptr> &groups)
{
    if (groups.empty())
        return nullptr;
    // 获取最优 group
    auto max_group = *max_element(groups.begin(), groups.end(),
                                  [&](const group_ptr &g1, const group_ptr &g2)
                                  {
                                      gyro_group_ptr lhs = GyroGroup::cast(g1);
                                      gyro_group_ptr rhs = GyroGroup::cast(g2);

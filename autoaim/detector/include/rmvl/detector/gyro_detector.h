...
public:
     ...
     * @return 识别信息结构体
     */
    DetectInfo detect(std::vector<group_ptr> &groups, cv::Mat &src, rm::PixChannel color,
...
private:
     ...
     */
    void matchOneGroup(group_ptr group, const std::vector<combo_ptr> &combos);
     ...
     * @return 推理得到的数据指标
     */
    std::unordered_map<size_t, size_t> ewTopsisInference(const group_ptr &group, const std::vector<combo_ptr> &combos);
};

...
void LightBlob::calcAccurateInfo(float lw_ratio, const vector<Point> &contour)
{
    // angle 向量
    ...
    // 轮廓分割
    vector<Point> up_contour, down_contour;
    ...
    if (lw_ratio < light_blob_param.RATIO_THRESHOLD)
    {
        Vec4f line_vec = {sin_angle, -cos_angle, _center.x, _center.y};
        _top = *min_element(up_contour.begin(), up_contour.end(),
                             [&line_vec](const Point &lhs, const Point &rhs)
                             {
                                 return getDistance(line_vec, lhs, false) < getDistance(line_vec, rhs, false);
                             });
        _bottom = *min_element(down_contour.begin(), down_contour.end(),
                                [&line_vec](const Point &lhs, const Point &rhs)
                                {
                                    return getDistance(line_vec, lhs, false) < getDistance(line_vec, rhs, false);
                                });
    ...
    {
        // 寻找上下顶点
        Vec4f line_vec = {cos_angle, sin_angle, _center.x, _center.y}; // 方向为角度向量的法向量
        _top = *max_element(up_contour.begin(), up_contour.end(),
                             [&line_vec](const Point &lhs, const Point &rhs)
                             {
                                 return getDistance(line_vec, lhs, false) < getDistance(line_vec, rhs, false);
                             });
        _bottom = *max_element(down_contour.begin(), down_contour.end(),
                                [&line_vec](const Point &lhs, const Point &rhs)
                                {
                                    return getDistance(line_vec, lhs, false) < getDistance(line_vec, rhs, false);
                                });
    ...
    _corners = {_bottom + Point2f(-half_w * cos_angle, -half_w * sin_angle),
                 _top + Point2f(-half_w * cos_angle, -half_w * sin_angle),
                 _top + Point2f(half_w * cos_angle, half_w * sin_angle),
...
shared_ptr<LightBlob> LightBlob::make_feature(vector<Point> &contour)
{
    ...
    // 根据斜率排除误识别
    if (abs(angle) > light_blob_param.MAX_ANGLE)
        return nullptr;
    // 获取灯条长宽比例
    ...
    // 长度过小，排除
    if (rotated_rect.size.height < light_blob_param.MIN_LENGTH)
        return nullptr;
    //  判断为远处灯条
    if (rotated_rect.size.height < light_blob_param.CLOSE_LENGTH)
    {
        // 比例判断
        if (lw_ratio > light_blob_param.MAX_FAR_RATIO || lw_ratio < light_blob_param.MIN_FAR_RATIO)
            return nullptr;
    }
    // 判断为近处灯条
    else
    {
        // 比例判断
        if (lw_ratio > light_blob_param.MAX_CLOSE_RATIO)
            return nullptr;
    }
    // 构造返回

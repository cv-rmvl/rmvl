...
void GyroGroup::sync(const ImuData &imu_data, double tick)
{
    ...
    for (auto p_tracker : _trackers)
    {
        auto robot_type = to_robot_type(p_tracker->state().at("robot"));
        if (robot_type != RobotType::UNKNOWN)
            robot_type_vec.push_back(robot_type);
    }
    if (robot_type_vec.empty())
        _state["robot"] = "unknown";
    else

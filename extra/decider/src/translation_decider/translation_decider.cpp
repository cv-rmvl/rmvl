...
TranslationDecider::Info TranslationDecider::decide(const std::vector<tracker::ptr> &trackers, tracker::ptr last_target,
                                                    cv::Mat src, const CompensateInfo &compensate_info,
    ...
    if (info.target == nullptr)
        info.target = getClosestTracker(src, trackers);
    // 判断是否正在追踪
    if (info.target != nullptr) {
        ...
        cv::Point2f angle_dKt(dKt[0], dKt[1]);
        cv::Point2f angle_dB(dB[0], dB[1]);

...
GyroPredictor::Info GyroPredictor::predict(const std::vector<group::ptr> &groups, const std::unordered_map<tracker::ptr, double> &tof) {
    // 预测信息
    Info info{};
    for (auto p_group : groups) {
        ...
        for (auto p_tracker : p_group->data()) {
            ...
            info.shoot_delay_prediction[p_tracker] = dBs_R;
        }
    }
    return info;
}

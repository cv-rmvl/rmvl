...
void PlanarTracker::initFilter()
{
    ...
    // 初始化距离滤波器
    _distance_filter.setR(cv::Matx22f::diag({0.01, 0.02})); // 距离为两帧差，误差为两倍
    _distance_filter.setQ(cv::Matx22f::diag({0.01, 0.01})); // 距离的过程噪声随意调整
    cv::Matx21f init_dis_vec = {first_combo->getExtrinsics().distance(), 0};
    _distance_filter.init(init_dis_vec, 1e-2);
    // 初始化运动滤波器，相对角度和角速度
    _motion_filter.setR(para::planar_tracker_param.R); // 距离为两帧差，误差为两倍
    _motion_filter.setQ(para::planar_tracker_param.Q); // 距离的过程噪声仔细调整
...
{
    // 设置状态转移矩阵
    _distance_filter.setA(cv::Matx22f{1, 1,
                                      0, 1});
    // 距离变化速度 / 差分

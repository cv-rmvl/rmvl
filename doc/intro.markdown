...
- **RMVL 2.x** —— *2024.09* 发布<span style="color: green">（已开源）</span>，多语言、跨平台、高兼容、低耦合的新版本。该系列首次加入了 Python 语言支持和 Windows 平台支持，可参考 @ref tutorials_python 。在架构上，将原先 @ref core 中的各类算法迁移至新增的 @ref algorithm 。在功能上，增设一组通信相关套件，包括基础通信设施 @ref io 与自研的通信中间件 LPSS @ref lpss ，不仅提供了跨平台的 I/O 协程设施、同步/异步串口通信、进程间通信、异步 HTTP Web 后端框架，还提供了基于 UDP/SHM 的高性能、低延迟的发布订阅通信方案，适用于机器人系统中的数据传输和控制指令的发送。

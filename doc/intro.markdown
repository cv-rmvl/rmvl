...
- **RMVL 2.x** —— *2024.09* 发布<span style="color: green">（已开源）</span>，在功能上，该系列首次加入了 Python 支持，可参考 @ref tutorials_python ，此外还为 Windows 用户提供了 MSVC 编译器的支持，使用 CPack 工具，为 Windows 以及 Debian 系 Linux 发行版制作安装包。在架构上，新增 @ref algorithm ，将原先 @ref core 中的各类算法迁移至该模块。新增基础通信设施 @ref io ，提供跨平台的 I/O 协程设施，以及同步、异步的串口通信、进程间通信等内容，并提供了异步的 HTTP Web 后端框架。新增了自研的通信中间件 LPSS @ref lpss ，提供了基于 UDP 协议的高性能、低延迟的发布订阅通信方案，适用于机器人系统中的数据传输和控制指令的发送。

#### 使用对象与基本情况

RMVL 为硬件设备二次开发、网络通信、串口通信以及运动、控制、视觉算法提供了相应的支持库，为工业、日常环境下某些特征的识别、追踪等提供了完整的流程，同时为 RoboMaster 参赛者提供有关装甲板识别与运动追踪、能量机关识别与运动追踪等自动瞄准的内容，具体可提供的内容如下

- 机器人在一些经典的、特有的工作环境下的功能需求，涉及到运动追踪、视觉识别等内容
- 控制、通信、数据结构的基础工具库
- 涉及 RoboMaster 的步兵机器人、英雄机器人、哨兵机器人和空中机器人 4 类射击型机器人的绝大部分功能
...
- @ref core (**core**) —— 包含主要的开发工具、宏定义、版本管理等内容，以及 rm::Exception 异常管理模块就在此处定义。

- @ref camera (**camera**) —— 目前包含 4 种相机厂商 SDK 的二次开发工具库，具体可参考下表。

  <div class="full_width_table">

  <center>

  表 1: 相机厂商 SDK 二次开发工具库
...
- @ref lpss (**lpss**) —— 基于 UDP 协议的高性能、低延迟的发布订阅通信方案，适用于机器人系统中的数据传输和控制指令的发送。
...
#### 跨平台支持 {#intro_cross_platform}

RMVL 支持 Linux 和 Windows 两大主流操作系统，并且支持 GCC、Clang、MSVC 三大主流编译器，用户可以在任意平台上使用 RMVL 进行开发和部署。RMVL 的跨平台支持主要通过 CMake 构建系统实现，CMake 能够自动检测用户的操作系统和编译器，并生成相应的构建文件，从而确保 RMVL 在不同平台上的兼容性和性能。

最能体现 RMVL 跨平台支持的内容是其基础通信设施 @ref io ，它们提供了跨平台的 I/O 协程设施，以及同步、异步的串口通信、进程间通信等内容，例如以下代码片段展示了 RMVL 中的串口通信模块在 Linux 和 Windows 平台上的统一使用方式：
...
int main() {
    // 打开 '/dev/ttyUSB0' 串口，波特率为 115200，非阻塞模式
    // Windows 下可能需要改为 "COM3" 或其他串口名称
    SerialPort serial("/dev/ttyUSB0", BaudRate::BR_115200, SerialReadMode::NONBLOCK);

    // 发送数据并读取响应
    serial.write("Hello, RMVL!");
    std::string res = serial.read();
    if (res.empty()) {
        printf("No data received.");
    } else {
        printf("Received: %s", res.c_str());
    }
}
```
...
auto p_compensator = rm::GravityCompensator::make_compensator();
auto p_predictor = rm::ArmorPredictor::make_predictor();
auto p_decider = rm::TranslationDecider::make_decider();
